...
function parseNumber(num) {
  return Number(String(num).replace(/\,|\;|\$|\s/g, ""));
}
  ...
  programs.forEach(function (pcode) {
    ...
    (program.required_yes || []).forEach(function (answerToCheck) {
      if (answers[answerToCheck] === false) {
        console.log(
          pcode + " checks " + answerToCheck + " and sees false instead of true"
        );
        muteProgram = true;
      }
    });
    ...
    (program.required_no || []).forEach(function (answerToCheck) {
      if (answers[answerToCheck] === true) {
        console.log(
          pcode + " checks " + answerToCheck + " and sees true instead of false"
        );
        muteProgram = true;
      }
    });
    ...
    answersToEval.forEach(function (answerToCheck) {
      if (
        typeof answers[answerToCheck] !== "undefined" &&
...
$(document).ready(function () {
  ...
  if (lang_options.length > 1 && window.location.search.indexOf("lang=") > -1) {
    select_lang = window.location.search
      .split("lang=")[1]
      .split("&")[0]
      .toLowerCase();
  ...
  Object.keys(questions).forEach(function (qcode) {
    var q = questions[qcode],
      lang_src =
        select_lang == "en"
          ? q
          : questions[qcode][select_lang] || { input: {} },
      shell = outer_shell;
    ...
    if (q.input) {
      ...
      if (lang_src.input.label || q.input.label) {
        formplace.append(
          $("<label>")
            .attr("for", q.input.name)
            .text(lang_src.input.label || q.input.label)
        );
      }
      if (lang_src.input.examples || q.input.examples) {
        formplace.append(
          $("<small>")
            ...
            .text(
              language_defaults[select_lang].examples +
                ": " +
                (lang_src.input.examples || q.input.examples)
            )
        );
      }
      ...
      formplace.append(
        $("<input>")
          .attr("type", "text")
          .attr("class", "input form-control form-control-lg")
          .attr("name", q.input.name)
          .attr("id", q.input.name)
      );
      formplace.append(
        $("<button>")
      ...
      if (lang_src.skippable || q.skippable) {
        var skip_label =
          q.skippable && q.skippable.length
            ? lang_src.skippable || q.skippable
      ...
      var select = $("<select>")
        .attr("id", q.select.name)
        .attr("name", q.select.name);

      select.append($("<option>").attr("value", "none").text("No selection"));
      for (key in q.select.options) {
        select.append(
          $("<option>").attr("value", key).text(q.select.options[key])
        );
      }
      select.val("none");
      qdiv.append(select);
      ...
      var yes_label =
          lang_src.yes_text || language_defaults[select_lang].yes_text,
        not_sure_label =
          q.skippable && q.skippable.length
            ? lang_src.skippable || q.skippable
            : language_defaults[select_lang].not_sure,
    ...
    $(q)
      ...
      .click(function (e) {
        ...
        answers[q.id] = questions[q.id].input
          ? $(q)
              .find('input[name="' + questions[q.id].input.name + '"]')
              .val()
          : true;
